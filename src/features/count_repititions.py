...
#Dropping the rest bc we only count reps
df = df[df["label"] != "rest"]

#Counting sum of squares again
...
#Just ploting each piece of data for bench just to viziualize
plot_df = bench_df
...
#We are smoothing out our r data using lowPass
column = "acc_r"
LowPass.low_pass_filter(
    bench_set, col = column, 
...
def count_reps(dataset, cutoff = 0.4, order = 10, column = "acc_r"):
    ...
    #Function to calculate the relative extrema of data
...
#Creates new colum reps, puts 5 for heavy 10 for not
df["reps"] = df["category"].apply(lambda x: 5 if x == "heavy" else 10)
#rep df is label category set and reps
...
error = mean_absolute_error(rep_df["reps"], rep_df["reps_predicted"]).round(2)
